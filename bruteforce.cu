...
__global__ void kernel_to_compute_optimal_match(int chunck_per_cycle, int num_perm_per_thread, lint nfact, int n, float *A, float *B, float (*metric)(int , float* , float *, int* ), float * obj_vals, lint * obj_perms , double * local_sums, int *local_num_perms ){
    ...
    for (int i = 0; i < num_perm_per_thread ; i++){
        ...
        if (ix < nfact){
            ...
            local_sum = local_sum + (double) val;
            local_count++;
...
int main(int argc,char *argv[]){
	...
	float val1;
        if (norm_to_use == 1){
        	compute_optimal_match(n, A, B, &L1_norm , bestperm, &val , &val1);
        }
        if (norm_to_use == 2){
	...
	printf("CPU Opt Mean  Val = %f\n", val1);
        ...
        lint * d_obj_perms;
	double * d_local_sums;
	int * d_local_num_perms;

        float * h_obj_vals = (float *) malloc( chunck_per_cycle*sizeof(float) );
        lint * h_obj_perms = (lint *) malloc( chunck_per_cycle*sizeof(lint) );

	double * h_local_sums = (double *) malloc( chunck_per_cycle*sizeof(double) );
        int * h_local_num_perms = (int *) malloc( chunck_per_cycle*sizeof(int) );
	...
	double mean_gpu_val = 0.0;
        ...
        for (int i = 0 ; i < chunck_per_cycle ; i++){
            ...
            mean_gpu_val = mean_gpu_val + h_local_sums[i];
            if (val < best_gpu_val){
                best_gpu_val  = val;
                best_ix = i;
            }
